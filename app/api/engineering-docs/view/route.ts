...
export async function GET(request: NextRequest): Promise<Response> {
  ...
  if (!docId) {
    return NextResponse.json({ error: "No docId provided" }, { status: 400, headers: noCacheHeaders });
  }
  ...
  try {
    ...
    if (docs.length === 0) {
      ...
      return NextResponse.json({ error: "Document not found", docId }, { status: 404, headers: noCacheHeaders });
    }
    ...
    return NextResponse.json({ error: "Failed to generate document URL" }, { status: 500, headers: noCacheHeaders });
  } catch (error) {
    console.error("View doc error:", error);
    return NextResponse.json({ error: "Failed to retrieve document" }, { status: 500, headers: noCacheHeaders });
  }
}
