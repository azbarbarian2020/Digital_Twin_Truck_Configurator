...
export async function GET(): Promise<Response> {
  try {
    ...
    return NextResponse.json({ error: "Failed to fetch engineering docs" }, { 
      status: 500,
      headers: {
        'Cache-Control': 'no-store, no-cache, must-revalidate',
        'Pragma': 'no-cache',
...
export async function DELETE(request: Request): Promise<Response> {
  ...
  (async () => {
    try {
      ...
      // Step 2: Delete validation rules for this doc
      await sendProgress('delete_rules', 'active', 'Removing validation rules...');
      console.log(`[DELETE] About to delete rules for DOC_ID: ${docId}`);
      console.log(`[DELETE] Query: DELETE FROM ${getFullTableName('VALIDATION_RULES')} WHERE DOC_ID = '${docId.replace(/'/g, "''")}'`);
      
      const deleteResult = await query<{ DELETED: number }>(`
        SELECT COUNT(*) as DELETED FROM ${getFullTableName('VALIDATION_RULES')} WHERE DOC_ID = '${docId.replace(/'/g, "''")}'
      `);
      const rulesCount = deleteResult[0]?.DELETED || 0;
      console.log(`[DELETE] Found ${rulesCount} rules to delete for DOC_ID: ${docId}`);
      
      const deleteQuery = `DELETE FROM ${getFullTableName('VALIDATION_RULES')} WHERE DOC_ID = '${docId.replace(/'/g, "''")}'`;
      console.log(`[DELETE] Executing: ${deleteQuery}`);
      await query(deleteQuery);
      console.log(`[DELETE] Delete query executed for DOC_ID: ${docId}`);
      await sendProgress('delete_rules', 'done', `Removed ${rulesCount} rules`);

      // Step 3: Remove PDF from stage
